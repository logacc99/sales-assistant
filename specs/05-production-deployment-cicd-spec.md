...
| :--- | :--- | :--- |
| **Port Conflict (`already using a port required by your task` / `bind: address already in use`)** | ECS rolling update defaults (Min 100%, Max 200%) attempt to spin up task #2 before stopping task #1. On a single EC2 instance, two containers cannot bind to host port `8000` simultaneously. | 1. In ECS Service settings, expand **Deployment options** and set **Minimum running tasks in percent** to `0%` and **Maximum** to `100%`. <br>2. Ensure service desired count is strictly `1`. <br>3. Stop any orphaned tasks in the ECS console or rogue containers (`docker ps`) via SSM. <br>4. Restart ECS agent if needed: `sudo systemctl restart ecs`. |
| **Disk Exhaustion (`CannotPullContainerError: failed to extract layer ... no space left on device`)** | Default EC2 root EBS volume is only 8 GiB (~6.8 GiB usable). With OS files (~2.7 GiB) and 2 GiB swapfile, only ~2.1 GiB remains free. Heavy application image layers (PyTorch, Crawl4AI/Patchright) exceed available disk space during decompression. | 1. Ensure EC2 instance root volume is provisioned as **30 GiB gp3** (100% Free Tier eligible). <br>2. On existing instance: Modify volume to 30 GiB in AWS Console, then run `sudo growpart /dev/nvme0n1 1` and `sudo resize2fs /dev/root` (or `sudo xfs_growfs /`). <br>3. Clean broken partial layer downloads: `sudo docker system prune -af`. |
| **Out-Of-Memory (OOM) Termination (Exit Code 137)** | Container exceeded physical RAM limit on `t3.micro`. | Ensure the 2GB swapfile was provisioned via EC2 User Data (`free -m` in instance terminal). Set `memoryReservation: 256` so Docker can place smoothly and swap idle pages. |
| **EC2 Instance Not Registering in ECS Cluster** | Incorrect cluster name in `/etc/ecs/ecs.config`, Docker/ECS agent not running, or missing `ecsInstanceRole`. | Connect via SSM Session Manager. Verify `/etc/ecs/ecs.config` contains `ECS_CLUSTER=sales-assistant-cluster`. Check Docker status (`systemctl status docker`) and ECS agent container (`docker ps -a`). Verify agent logs: `docker logs ecs-agent`. |
| **Connection Timed Out on `http://<IP>:8000`** | Security Group missing inbound rule for port 8000, or public IP not assigned. | Verify `sales-assistant-ec2-sg` permits inbound TCP 8000 from caller CIDR. Verify EC2 instance has an Auto-assigned Public IPv4 address or Elastic IP. |
| **`CannotPullContainerError: ...: not found`** | Image with tag `latest` does not exist in ECR yet. | Push the container image via GitHub Actions (`workflow_dispatch`) or locally via `docker push <AWS_ACCOUNT_ID>.dkr.ecr.ap-southeast-1.amazonaws.com/sales-assistant:latest`. |
| **`Container instance lacks required attribute com.amazonaws.ecs.capability.task-iam-role`** | Task definition specified `taskRoleArn` on a self-managed EC2 instance without the metadata credential proxy. | Leave **Task role** blank/empty in the Task Definition so the container inherits permissions from `ecsInstanceRole` directly via EC2 metadata service. |
| **`Could not assume role with OIDC: Not authorized to perform sts:AssumeRoleWithWebIdentity`** | 1. OIDC Identity Provider (`token.actions.githubusercontent.com`) missing official GitHub intermediate/root CA thumbprints (`6938fd4d98bab03faadb97b34396831e3780aea1`, `1c5860f56b130439f181f4f0c69fa1aa14e0d6ff`). <br>2. Role trust policy `sub` claim casing or branch pattern mismatch. | 1. Add both official GitHub thumbprints to the IAM OIDC provider using `aws iam update-open-id-connect-provider-thumbprint`. <br>2. Ensure trust policy condition evaluates `token.actions.githubusercontent.com:sub` with `repo:logacc99/sales-assistant:*` (or `repo:*`). <br>3. Verify Audience contains `sts.amazonaws.com`. |
| **`ResourceInitializationError: unable to retrieve secret`** | Missing `secretsmanager:GetSecretValue` on `ecsTaskExecutionRole`. | Verify secret name and attach inline policy allowing `secretsmanager:GetSecretValue` on secret ARN to `ecsTaskExecutionRole`. |
